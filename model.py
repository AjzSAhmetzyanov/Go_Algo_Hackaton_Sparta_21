...
def create_dataset(i):
  trade = company[i].tradestats(date=f'2023-12-0{day-3}', till_date=f'2023-12-0{day-1}')
  trade.rename(columns={'systime':'systime_trade'}, inplace=True)
  order = company[i].orderstats(date=f'2023-12-0{day-3}', till_date=f'2023-12-0{day-1}')
  order.rename(columns={'systime':'systime_order'}, inplace=True)
  obs = company[i].obstats(date=f'2023-12-0{day-3}', till_date=f'2023-12-0{day-1}')
...
def mi_method(X, y):
  ...
  return mi_importance_df

# Пермутационные важности:
def perm_method(X, y, rf_model):
...
# Модель:
def model(X, y, data_, result, sequence_length, scaler):
...
# Получение 30 значенний:
def get_data(future_predictions_df):
...
# Ищем даты для покупки и продажи
def get_bidding_date(future_predictions_df):
...
def strategy(future_predictions_df):
...
def start_model():
  ...
  for i in range(0, 1):
    ...
    # RandomForestRegressor metod
    feature_importance_df = random_forest_method(X, y)[0]

    # Метод одномерного анализа вариации (ANOVA):
    anova_importance_df = anova_method(X, y)

    # Метод взаимной информации:
    mi_importance_df = mi_method(X, y)

    # Пермутационные важности:
    perm_importance_df = perm_method(X, y, random_forest_method(X,y)[1])
    ...
    # Применение модели
    future_predictions_df = model(X, y, data_, result, sequence_length, scaler)

    # Применение стратегии
    future_predictions_df = strategy(future_predictions_df)

    # Находим точки покупки и продажи
    entry_date, exit_date = get_bidding_date(future_predictions_df)
    ...
    result_data = get_data(future_predictions_df)
